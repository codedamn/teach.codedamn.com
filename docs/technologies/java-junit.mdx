...
title: How to create an interactive Java lab?
---
...
class TestFile {
    ...
    public void test1() {
        try {
			// Use reflection API for testing
			// cheatsheet: https://javapapers.com/core-java/java-reflection-cheat-sheet/
			// This helps in partial test passing
            Class UserFirstClass = Class.forName("< user class name >");
            Constructor UserFirstClassConstructor = UserFirstClass.getDeclaredConstructor();
            Object colony = UserFirstClassConstructor.newInstance();
            Method getTestMethod123 = UserFirstClass.getMethod("< some method inside class >");

            int val1 = (int) getTestMethod123.invoke(colony);
...
-   Also note that the tests here are run in order through `@TestMethodOrder(OrderAnnotation.class)` and `@Order(1)` decorators. This is important if you want to map the results of these tests (passed/failed) correctly with the tests you added in challenges tab earlier.
